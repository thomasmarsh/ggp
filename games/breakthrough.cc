...
//      BreakthroughMinimax
//      BreakthroughNegamax
//      BreakthroughNegascout
//      Random<BTState>
//
//----------------------------------------------------------------------------

#define PLAYER_X BreakthroughUCT
#define PLAYER_Y BreakthroughUCT
...
typedef MonteCarlo<
                BTState,
                AREA,
                MAX_ITER> BreakthroughMC;

typedef BasicMinimax<
                BTState,
                Minimax<BTState>,
                MAX_PLIES > BreakthroughMinimax;

typedef BasicMinimax<
                BTState,
                Negamax<BTState>,
                MAX_PLIES > BreakthroughNegamax;

typedef BasicMinimax<
                BTState,
                Negascout<BTState>,
                MAX_PLIES > BreakthroughNegascout;

typedef Game<   BTState,
                PLAYER_X,
                PLAYER_Y > BreakthroughGame;
...
template<> MemoryPool<BreakthroughUCT::Node> BreakthroughUCT::Node::pool(MAX_ITER);
...
void test3() {
        Contest<BreakthroughGame> c;
        c.run();
}

int main(int argc, char **argv) {
        LOG("sizeof(UCTNode) = " << sizeof(BreakthroughUCT::Node));
        srandom(time(NULL));
        test2(NULL);
        //test3();
        return 0;
