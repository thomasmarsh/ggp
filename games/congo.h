...
static const uint64_t INVALID       = 0xFF80808080808080,
...
static uint64_t ADJACENT_MOVES[NUM_POSITIONS],
...
static uint64_t LION_SIGHT[9][9];
...
struct MoveList {
        ...
        void _add(Color c, Piece target, bitboard bb, int a, int b) {
                move[count].a = a;
                move[count].b = b;
                move[count].c = c;
                move[count].undo_a = b;
                ...
                move[count].capture = target;
                ++count;
        }

        void add(Color c, uint8_t square[], bitboard bb, int k, uint64_t d) {
                uint64_t j, n=d;
                while (n) {
                        j = n & (~n+1);
                        _add(c, (Piece) square[log2(j)], bb, k, log2(j));
                        n ^= j;
                }
        }
...
struct State {
        ...
        void promote(Color c, uint64_t promo, uint64_t data, int x) {
                uint64_t j, n=data&promo;
                int i;
                while (n) {
                        j = n & (~n+1);
                        i = log2(j);
        ...
        void moves(MoveList &l, Color c) {
                uint64_t j, friendly=((c==BLACK)?occupied_black.data:occupied_white.data),
                         n = occupied.data;
                int i;
                while (n) {
                        j = n & (~n+1);
